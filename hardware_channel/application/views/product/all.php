			    ...
			    <div class="box-header" style="width:50%">
                  <form action="<?php echo base_url(); ?>product/search_product" method="post">
                    <div class="input-group">
                      <input type="text" name="searchproduct" placeholder="Search by Product Name" class="typeahead form-control"  autocomplete="off" spellcheck="false" />
                      <span class="input-group-btn">
                        <button type="submit" class="btn btn-block btn-primary btn-sm">Search Product</button>
                      </span>
                    </div>
                  </form>
                </div>	
                ...
                <div class="box-body table-responsive no-padding">
                  <table class="table table-hover">
                    ...
                    <tr>
                      ...
                      <th><a href="<?php echo base_url(); ?>product/delete/<?php echo $item->pid; ?>/<?php echo $offset; ?>" onclick="return confirm('Are you sure you want to delete?')"><span class="label label-danger">Delete</span></a> &nbsp; <a href="<?php echo base_url(); ?>product/edit/<?php echo $item->pid; ?>/<?php echo $offset; ?>"><span class="label label-warning">Edit</span></a></th>
                    </tr>
                    <?php $i++;} ?>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('input.typeahead').typeahead({
            name: 'product',
			items: '20',
            local: [<?php foreach($prod as $item) { ?> '<?php $PName = str_replace("'", "", $item->pname); echo $PName;?>', <?php } ?>]
        });
    });  
</script>